<?php
/**
 * GET: animales del cliente logueado (o todos si staff)
 * POST: registrar animal (solo cliente)
 */
...
if ($nombre === '' || $razaId <= 0) {
    api_json(['ok' => false, 'error' => 'Nombre y raza son obligatorios'], 400);
}

if ($sexo !== 'M' && $sexo !== 'F') {
    api_json(['ok' => false, 'error' => 'Sexo debe ser M o F'], 400);
}
...
$st->execute([$razaId]);
if (! $st->fetch()) {
    api_json(['ok' => false, 'error' => 'Raza no válida'], 400);
}
...
if ($animalIdEdit > 0) {
    ...
    api_json(['ok' => true, 'animalId' => $animalIdEdit]);
}
...
api_json(['ok' => true, 'animalId' => $nextId]);
